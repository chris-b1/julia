    ...
    if (!cache_with_orig && mt) {
        ...
        if (!cache_with_orig && guards > 0) {
            ...
            for (i = 0, l = jl_array_len(temp); i < l; i++) {
                ...
                if (other != definition) {
                    jl_svecset(guardsigs, guards, matc->metharg);
                    guards++;
                    // alternative approach: insert sentinel entry
                    //jl_typemap_insert(cache, parent, (jl_tupletype_t*)matc->metharg,
...
jl_method_instance_t *jl_get_specialization1(jl_tupletype_t *types JL_PROPAGATES_ROOT, size_t world, size_t *min_valid, size_t *max_valid, int mt_cache)
{
    ...
    jl_svec_t *env = match->methsp;
    jl_tupletype_t *ti = match->metharg;
    jl_method_instance_t *nf = NULL;
...
static jl_method_match_t *make_method_match(jl_value_t *metharg, jl_svec_t *methsp, jl_method_t *method, int fully_covers)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_method_match_t *match = (jl_method_match_t*)jl_gc_alloc(ptls, sizeof(jl_method_match_t), jl_method_match_type);
    match->metharg = metharg;
    match->methsp = methsp;
    match->method = method;
...
static int ml_matches_visitor(jl_typemap_entry_t *ml, struct typemap_intersection_env *closure0)
{
    ...
    closure->matc = make_method_match(closure->match.ti, closure->match.env, meth, closure->match.issubty);
                              ...
                              size_t *min_valid, size_t *max_valid, int *has_ambiguity)
{
    ...
    if (((jl_datatype_t*)unw)->isdispatchtuple) {
        ...
        if (entry) {
            ...
            env.matc = make_method_match(env.match.ti, env.match.env, meth, 1);
    ...
    *max_valid = env.max_valid;
    ...
    if (len > 1) {
            ...
            if (!all_subtypes && minmax) {
                ...
                for (i = 0; i < len; i++) {
                    ...
                    if (!matc->fully_covers) {
                        jl_method_t *m = matc->method;
                        if (jl_type_morespecific((jl_value_t*)minmaxm->sig, (jl_value_t*)m->sig))
                            matc->sentinel = 1; // put a sentinel value here for sorting
                        else
                            all_subtypes = 0;
                    }
                }
        ...
        for (i = 1; i < len; i++) {
            ...
            int subt = env.matc->fully_covers || env.matc->sentinel;
            if (minmax != NULL && subt) {
            ...
            for (j = 0; j < i; j++) {
                ...
                int subt2 = matc2->fully_covers || matc2->sentinel;
                if (!subt2 && subt)
                    break;
                if (subt == subt2)
        ...
        if (!minmax) {
            ...
            for (; i > 0; i--) {
                ...
                if (j == i)
                    break;
                env.matc->fully_covers = 0x0;
                env.matc->sentinel = 0x1;
        ...
        if (minmax_ambig && !include_ambiguous) {
            for (i = 0; i < len; i++) {
                jl_method_match_t *matc = (jl_method_match_t*)jl_array_ptr_ref(env.t, i);
                if (matc->fully_covers || matc->sentinel) {
                    skip[i] = 1;
                }
            }
        }
            ...
            for (i = 0; i < len; i++) {
                jl_method_match_t *matc = (jl_method_match_t*)jl_array_ptr_ref(env.t, i);
                if (minmax != matc && (matc->fully_covers || matc->sentinel)) {
                    skip[i] = 1;
                }
            }
        }
        ...
        for (i = 0; i < len; i++) {
            ...
            jl_method_match_t *matc = (jl_method_match_t*)jl_array_ptr_ref(env.t, i);
            jl_method_t *m = matc->method;
            int subt = matc->fully_covers; // jl_subtype((jl_value_t*)type, (jl_value_t*)m->sig)
            ambig_groupid[i] = i;
            int disjoint = 1;
            for (j = i; j > 0; j--) {
                jl_method_match_t *matc2 = (jl_method_match_t*)jl_array_ptr_ref(env.t, j - 1);
                jl_method_t *m2 = matc2->method;
                int subt2 = matc2->fully_covers; // jl_subtype((jl_value_t*)type, (jl_value_t*)m2->sig)
                if (skip[j - 1]) {
                    ...
                    assert(matc2->fully_covers || matc2->sentinel);
                    disjoint = 0;
        ...
        for (i = 0; i < len; i++) {
            jl_method_match_t *matc = (jl_method_match_t*)jl_array_ptr_ref(env.t, i);
            if (matc->fully_covers) { // jl_subtype((jl_value_t*)type, (jl_value_t*)m->sig)
    ...
    if (cache_result && ((jl_datatype_t*)unw)->isdispatchtuple) { // cache_result parameter keeps this from being recursive
        if (len == 1 && !*has_ambiguity) {
            ...
            jl_svec_t *tpenv = env.matc->methsp;
