...
function abstract_call_gf_by_type(interp::AbstractInterpreter, @nospecialize(f), argtypes::Vector{Any}, @nospecialize(atype), sv::InferenceState,
    ...
    for i in 1:napplicable
        ...
        if splitunions
            ...
            this_rt, edgecycle1, edge = abstract_call_method(interp, method, sig, match.methsp, multiple_matches, sv)
            edgecycle |= edgecycle1::Bool
            if edge !== nothing
                push!(edges, edge)
            end
...
is_method_pure(match::MethodMatch) = is_method_pure(match.method, match.metharg, match.methsp)
